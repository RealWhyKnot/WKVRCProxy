...
.SYNOPSIS
  ...
  docs/wiki/Changelog.md (synced to the GitHub Wiki) so the changelog stays
...
# (see WKVRCProxy.UI.csproj <EmbeddedResource>). The docs/wiki copy is mirrored
# to the GitHub Wiki by .github/workflows/wiki-sync.yml.
$RootChangelog = Join-Path $RepoRoot 'CHANGELOG.md'
$WikiChangelog = Join-Path $RepoRoot 'docs/wiki/Changelog.md'

if (-not (Test-Path $RootChangelog)) { throw "CHANGELOG.md not found at $RootChangelog" }
if (-not (Test-Path $WikiChangelog)) { throw "docs/wiki/Changelog.md not found at $WikiChangelog" }
...
if ($Mode -eq 'Append') {
    ...
    Write-Host "Appended $included entr(ies) to both CHANGELOG.md and docs/wiki/Changelog.md."
    return
}
