...
<template>
  <div class="p-8 h-full flex flex-col space-y-8">
    ...
    <div class="flex-grow bg-white/[0.02] border border-white/5 rounded-[32px] overflow-hidden backdrop-blur-3xl shadow-2xl flex flex-col font-mono text-[9px]">
      <div class="px-8 py-4 bg-white/[0.01] border-b border-white/5 flex justify-between text-white/45 font-black uppercase tracking-[0.3em] italic">
        <span>Event Log</span>
      ...
      <div ref="scrollContainer" class="flex-grow overflow-y-auto p-8 space-y-1.5 no-scrollbar leading-relaxed scroll-smooth">
        <div v-for="(log, i) in appStore.logs" :key="i" class="flex gap-4 border-l border-transparent hover:border-blue-500/40 hover:bg-white/[0.03] px-4 transition-all py-0.5 rounded-r-lg group/item">
          <span class="text-white/35 shrink-0 group-hover/item:text-white/55 font-bold tabular-nums">{{ log.Timestamp.split('T')[1]?.split('.')[0] }}</span>
          <span :class="logLevelClasses[log.Level]" class="shrink-0 w-20 uppercase tracking-widest">[{{ logLevelNames[log.Level] }}]</span>
          <span class="text-white/65 group-hover/item:text-white/85 transition-colors break-all">{{ log.Message }}</span>
        </div>
        <div v-if="appStore.logs.length === 0" class="text-white/25 italic py-20 text-center uppercase tracking-[0.8em] font-black">No logs yet</div>
      </div>
    </div>
  </div>
</template>
